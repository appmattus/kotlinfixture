...
== `repeatCount` - List and map length
...
== `subType` - Subclass used when resolving a superclass
...
== `factory` - Customise generation of a class
...
== `property` - Customise generation of class properties
...
== `random` - Providing a seed for `Random`
